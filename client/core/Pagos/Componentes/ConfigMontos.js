...
const ConfigMonto = ({ credentials, onSelectYear, selectedYear, configMonto,setConfigMonto }) => {
  ...
  const fetchConfigMontos = async (signal) => {
    ...
    try {
      ...
      // console.log('fetchConfigMontos - raw data:', data);
      if (data.error) {
        throw new Error(data.message);
      }
      const dataArray = Object.values( data)
      const processedData = dataArray
        .filter((row) => row && row.id != null && row.agno != null && row.monto != null)
        .map((row) => ({
          id: row.id,
          agno: parseInt(row.agno),
          monto: Number(row.monto),
        }));
      // console.log('fetchConfigMontos - processed data:', processedData);
      ...
      // console.log('fetchConfigMontos - updated rows:', processedData, 'paginationModel:', paginationModel);
    } catch (error) {
      // console.error('fetchConfigMontos - error:', error);
      setSnackbar({ open: true, message: error.message || 'Error al cargar los montos', severity: 'error' });
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async (id) => {
    ...
    if (row?.isNew) {
      ...
      // console.log('confirmDelete - updated rows:', rows.filter((r) => r.id !== id), 'paginationModel:', paginationModel);
      return;
    }
    try {
      ...
      setSnackbar({ open: true, message: 'Monto eliminado correctamente', severity: 'success' });
      onSelectYear?.(null);
      setConfigMonto?.(0);
      // Refresh data from API to ensure synchronization
      await fetchConfigMontos(new AbortController().signal);
      // console.log('confirmDelete - after fetchConfigMontos, rows:', rows, 'paginationModel:', paginationModel);
    } catch (error) {
      setSnackbar({ open: true, message: error.message || 'Error al eliminar el monto', severity: 'error' });
    }
  };
    ...
    (newRow, oldRow) =>
      new Promise((resolve, reject) => {
        ...
        if (Object.keys(errors).length > 0) {
          setSnackbar({ open: true, message: errors.agno || errors.monto, severity: 'error' });
          reject(oldRow);
          return;
        }
  ...
  const handleRowClick = (params) => {
    if (params.row && params.row.id !== 'new' && rowModesModel[params.row.id]?.mode !== GridRowModes.Edit) {
      // console.log('Seleccionando año:', params.row.agno);
      onSelectYear?.(params.row.agno);
      setConfigMonto?.(params.row.monto)
    }
  };
  ...
  const handleYes = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Monto guardado correctamente', severity: 'success' });
      resolve({ ...newRow, id: response.id || newRow.id, isNew: false });
      setPromiseArguments(null);
      fetchConfigMontos(new AbortController().signal);
    } catch (error) {
      setSnackbar({ open: true, message: error.message || 'Error al guardar el monto', severity: 'error' });
      reject(newRow);
      setPromiseArguments(null);
    }
  };
    ...
    () => [
      ...
      },
      {
        ...
        renderCell: (params) => {
          ...
          return value != null && value !== '' ? `$${Number(value).toLocaleString('es-CL')}` : '$0';
        },
      },
      {
        field: 'actions',
    ...
    <Card sx={{ maxWidth: 500, boxShadow: 3 }}>
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Typography variant="h6">Configuración de Montos Año: {selectedYear} Monto {configMonto}</Typography>
        </Box>
